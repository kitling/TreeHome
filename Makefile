...
$(BUILD)/icon.icn:
	@$(BANNERTOOL) makesmdh -s "$(APP_TITLE)" -l "$(APP_DESCRIPTION)" -p "$(APP_AUTHOR)" -i "$(APP_ICON)" $(ICON_FLAGS_ARGS) -o "$@"
...
ifneq ($(strip $(ICON_FLAGS)),)
	ICON_FLAGS_ARGS := -f "$(ICON_FLAGS)"
endif
